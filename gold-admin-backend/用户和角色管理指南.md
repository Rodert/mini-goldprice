# 👥 用户和角色管理 - 完整指南

## 📋 目录
1. [RBAC 权限模型](#rbac-权限模型)
2. [用户管理](#用户管理)
3. [角色管理](#角色管理)
4. [权限关系](#权限关系)
5. [API 使用示例](#api-使用示例)
6. [前端使用示例](#前端使用示例)
7. [实际操作步骤](#实际操作步骤)

---

## 🔐 RBAC 权限模型

### 权限关系图

```
┌──────────────┐       ┌──────────────┐       ┌──────────────┐
│   用户 User  │ ←────→ │  角色 Role   │ ←────→ │  菜单 Menu   │
└──────────────┘       └──────────────┘       └──────────────┘
     (N)                    (M)                     (P)
     
     多对多关系              多对多关系
```

### 三层权限模型

```
用户（AdminUser）
  ↓ （通过 user_roles 表关联）
角色（Role）
  ↓ （通过 role_menus 表关联）
菜单（Menu）
```

### 数据表设计

#### 1. admin_users - 用户表
```sql
CREATE TABLE admin_users (
  id INTEGER PRIMARY KEY,
  username VARCHAR(50) UNIQUE NOT NULL,    -- 用户名
  password VARCHAR(255) NOT NULL,          -- 密码（加密）
  real_name VARCHAR(50),                   -- 真实姓名
  phone VARCHAR(20),                       -- 手机号
  email VARCHAR(100),                      -- 邮箱
  avatar VARCHAR(255),                     -- 头像
  status TINYINT DEFAULT 1,                -- 状态 1:启用 0:禁用
  last_login_time DATETIME,                -- 最后登录时间
  last_login_ip VARCHAR(50),               -- 最后登录IP
  created_at DATETIME,
  updated_at DATETIME
);
```

#### 2. roles - 角色表
```sql
CREATE TABLE roles (
  id INTEGER PRIMARY KEY,
  name VARCHAR(50) NOT NULL,               -- 角色名称
  code VARCHAR(50) UNIQUE NOT NULL,        -- 角色代码
  description TEXT,                        -- 描述
  sort INT DEFAULT 0,                      -- 排序
  status TINYINT DEFAULT 1,                -- 状态
  created_at DATETIME,
  updated_at DATETIME
);
```

#### 3. user_roles - 用户角色关联表
```sql
CREATE TABLE user_roles (
  id INTEGER PRIMARY KEY,
  user_id INTEGER NOT NULL,                -- 用户ID
  role_id INTEGER NOT NULL,                -- 角色ID
  created_at DATETIME,
  UNIQUE(user_id, role_id)                 -- 防止重复分配
);
```

#### 4. role_menus - 角色菜单关联表
```sql
CREATE TABLE role_menus (
  id INTEGER PRIMARY KEY,
  role_id INTEGER NOT NULL,                -- 角色ID
  menu_id INTEGER NOT NULL,                -- 菜单ID
  created_at DATETIME,
  UNIQUE(role_id, menu_id)                 -- 防止重复分配
);
```

---

## 👤 用户管理

### 用户属性

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| username | string | ✅ | 用户名（唯一，3-50字符） |
| password | string | ✅ | 密码（6位以上，bcrypt加密） |
| real_name | string | ❌ | 真实姓名 |
| phone | string | ❌ | 手机号 |
| email | string | ❌ | 邮箱 |
| avatar | string | ❌ | 头像URL |
| status | int8 | ❌ | 状态（1:启用 0:禁用） |
| role_ids | []uint | ❌ | 角色ID列表 |

### 用户 API 接口

#### 1. 获取用户列表
```bash
GET /api/users?page=1&page_size=10&username=admin&status=1
```

**查询参数**:
- `page` - 页码（默认1）
- `page_size` - 每页数量（默认10）
- `username` - 用户名（模糊查询）
- `real_name` - 真实姓名（模糊查询）
- `status` - 状态（1:启用 0:禁用）

**响应示例**:
```json
{
  "code": 200,
  "data": {
    "list": [
      {
        "id": 1,
        "username": "admin",
        "real_name": "系统管理员",
        "phone": "13800138000",
        "email": "admin@example.com",
        "avatar": "",
        "status": 1,
        "last_login_time": "2025-11-04T10:00:00Z",
        "last_login_ip": "127.0.0.1",
        "created_at": "2025-11-04T10:00:00Z"
      }
    ],
    "total": 1,
    "page": 1,
    "size": 10
  }
}
```

#### 2. 获取用户详情
```bash
GET /api/users/:id
```

**响应示例**:
```json
{
  "code": 200,
  "data": {
    "user": {
      "id": 1,
      "username": "admin",
      "real_name": "系统管理员",
      "status": 1
    },
    "role_ids": [1, 2]  // 用户拥有的角色ID列表
  }
}
```

#### 3. 创建用户
```bash
POST /api/users
Content-Type: application/json
Authorization: Bearer <token>

{
  "username": "test001",
  "password": "123456",
  "real_name": "测试用户",
  "phone": "13800138001",
  "email": "test@example.com",
  "status": 1,
  "role_ids": [1, 2]
}
```

**响应**:
```json
{
  "code": 200,
  "message": "创建成功",
  "data": {
    "id": 2,
    "username": "test001",
    "real_name": "测试用户",
    "status": 1
  }
}
```

#### 4. 更新用户
```bash
PUT /api/users/:id
Content-Type: application/json

{
  "real_name": "测试用户2",
  "phone": "13800138002",
  "status": 1,
  "role_ids": [2, 3]  // 重新分配角色
}
```

#### 5. 删除用户
```bash
DELETE /api/users/:id
```

**注意**: 
- ⚠️ 不能删除ID为1的默认管理员
- ⚠️ 删除用户会同时删除用户角色关联

#### 6. 修改用户密码
```bash
PUT /api/users/:id/password
Content-Type: application/json

{
  "password": "new_password"
}
```

---

## 🎭 角色管理

### 预设角色

| ID | 角色名称 | 角色代码 | 说明 |
|----|----------|----------|------|
| 1 | 超级管理员 | super_admin | 拥有所有权限 |
| 2 | 总部店长 | head_manager | 管理所有店铺 |
| 3 | 单店店长 | shop_manager | 管理单个店铺 |
| 4 | 店员 | shop_staff | 只读权限 |
| 5 | 财务 | finance | 查看数据、导出 |

### 角色属性

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| name | string | ✅ | 角色名称 |
| code | string | ✅ | 角色代码（唯一） |
| description | string | ❌ | 角色描述 |
| sort | int | ❌ | 排序 |
| status | int8 | ❌ | 状态（1:启用 0:禁用） |
| menu_ids | []uint | ❌ | 菜单ID列表 |

### 角色 API 接口

#### 1. 获取角色列表
```bash
GET /api/roles?page=1&page_size=10
```

**响应示例**:
```json
{
  "code": 200,
  "data": {
    "list": [
      {
        "id": 1,
        "name": "超级管理员",
        "code": "super_admin",
        "description": "拥有所有权限",
        "sort": 1,
        "status": 1,
        "created_at": "2025-11-04T10:00:00Z"
      }
    ],
    "total": 5,
    "page": 1,
    "size": 10
  }
}
```

#### 2. 获取所有角色（不分页）
```bash
GET /api/roles/all
```

**用途**: 用于下拉选择框

**响应示例**:
```json
{
  "code": 200,
  "data": [
    {
      "id": 1,
      "name": "超级管理员",
      "code": "super_admin",
      "status": 1
    },
    {
      "id": 2,
      "name": "总部店长",
      "code": "head_manager",
      "status": 1
    }
  ]
}
```

#### 3. 获取角色详情
```bash
GET /api/roles/:id
```

**响应示例**:
```json
{
  "code": 200,
  "data": {
    "role": {
      "id": 1,
      "name": "超级管理员",
      "code": "super_admin",
      "description": "拥有所有权限"
    },
    "menu_ids": [1, 2, 3, 4, 5, 6]  // 角色拥有的菜单ID
  }
}
```

#### 4. 创建角色
```bash
POST /api/roles
Content-Type: application/json

{
  "name": "测试角色",
  "code": "test_role",
  "description": "测试用角色",
  "sort": 10,
  "status": 1,
  "menu_ids": [1, 2, 5]  // 分配菜单权限
}
```

#### 5. 更新角色
```bash
PUT /api/roles/:id
Content-Type: application/json

{
  "name": "测试角色2",
  "description": "更新后的描述",
  "status": 1,
  "menu_ids": [1, 2, 3, 5]  // 重新分配菜单
}
```

**注意**: 
- ⚠️ 不能修改 super_admin 角色

#### 6. 删除角色
```bash
DELETE /api/roles/:id
```

**注意**: 
- ⚠️ 不能删除 super_admin 角色
- ⚠️ 如果角色已分配给用户，不能删除

---

## 🔗 权限关系

### 权限流程图

```
用户登录
   ↓
查询用户信息
   ↓
查询用户角色（user_roles）
   ↓
查询角色菜单（role_menus）
   ↓
返回菜单树
   ↓
前端动态加载路由
```

### 权限计算示例

**示例用户**: test001

1. **用户基本信息**:
   ```json
   {
     "id": 2,
     "username": "test001",
     "real_name": "测试用户"
   }
   ```

2. **用户角色**（从 user_roles 表查询）:
   ```json
   {
     "user_id": 2,
     "role_ids": [2, 3]  // 总部店长 + 单店店长
   }
   ```

3. **角色菜单**（从 role_menus 表查询）:
   ```json
   {
     "role_2_menus": [1, 2, 5, 6],    // 总部店长的菜单
     "role_3_menus": [1, 5, 6]        // 单店店长的菜单
   }
   ```

4. **合并去重**:
   ```json
   {
     "menu_ids": [1, 2, 5, 6]  // 用户最终拥有的菜单
   }
   ```

5. **构建菜单树**:
   ```json
   [
     {
       "id": 1,
       "title": "系统管理",
       "children": [
         {"id": 2, "title": "用户管理"}
       ]
     },
     {
       "id": 5,
       "title": "价格管理",
       "children": [
         {"id": 6, "title": "价格列表"}
       ]
     }
   ]
   ```

---

## 💻 API 使用示例

### curl 命令示例

#### 1. 创建用户并分配角色
```bash
# 获取 token
TOKEN=$(curl -s -X POST http://localhost:8080/api/login \
  -H "Content-Type: application/json" \
  -d '{"username":"admin","password":"admin123"}' \
  | jq -r '.data.token')

# 创建用户
curl -X POST http://localhost:8080/api/users \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $TOKEN" \
  -d '{
    "username": "shop_manager_001",
    "password": "123456",
    "real_name": "店长001",
    "phone": "13800138001",
    "status": 1,
    "role_ids": [3]
  }'
```

#### 2. 修改用户角色
```bash
# 将用户从"单店店长"改为"总部店长"
curl -X PUT http://localhost:8080/api/users/2 \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $TOKEN" \
  -d '{
    "role_ids": [2]
  }'
```

#### 3. 创建自定义角色
```bash
# 先获取所有菜单
curl -H "Authorization: Bearer $TOKEN" \
  http://localhost:8080/api/menus | jq

# 创建角色并分配特定菜单
curl -X POST http://localhost:8080/api/roles \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $TOKEN" \
  -d '{
    "name": "价格管理员",
    "code": "price_admin",
    "description": "只管理价格",
    "status": 1,
    "menu_ids": [5, 6]
  }'
```

---

## 🎨 前端使用示例

### Vue 组件示例

#### 1. 用户列表组件
```vue
<template>
  <div>
    <!-- 搜索表单 -->
    <el-form :inline="true">
      <el-form-item label="用户名">
        <el-input v-model="queryParams.username" placeholder="请输入用户名" />
      </el-form-item>
      <el-form-item>
        <el-button type="primary" @click="getList">查询</el-button>
        <el-button @click="handleAdd">新增用户</el-button>
      </el-form-item>
    </el-form>

    <!-- 用户表格 -->
    <el-table :data="userList" border>
      <el-table-column prop="id" label="ID" width="80" />
      <el-table-column prop="username" label="用户名" />
      <el-table-column prop="real_name" label="真实姓名" />
      <el-table-column prop="phone" label="手机号" />
      <el-table-column label="状态">
        <template slot-scope="scope">
          <el-tag :type="scope.row.status === 1 ? 'success' : 'danger'">
            {{ scope.row.status === 1 ? '启用' : '禁用' }}
          </el-tag>
        </template>
      </el-table-column>
      <el-table-column label="操作" width="300">
        <template slot-scope="scope">
          <el-button size="mini" @click="handleEdit(scope.row)">编辑</el-button>
          <el-button size="mini" @click="handleAssignRole(scope.row)">分配角色</el-button>
          <el-button size="mini" type="danger" @click="handleDelete(scope.row)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>

    <!-- 分页 -->
    <el-pagination
      :total="total"
      :page-size="queryParams.page_size"
      :current-page="queryParams.page"
      @current-change="handlePageChange"
    />
  </div>
</template>

<script>
import { getUserList, deleteUser } from '@/api/user'

export default {
  data() {
    return {
      userList: [],
      total: 0,
      queryParams: {
        page: 1,
        page_size: 10,
        username: ''
      }
    }
  },
  created() {
    this.getList()
  },
  methods: {
    async getList() {
      const res = await getUserList(this.queryParams)
      this.userList = res.data.list
      this.total = res.data.total
    },
    handlePageChange(page) {
      this.queryParams.page = page
      this.getList()
    },
    handleAdd() {
      this.$router.push('/system/user/add')
    },
    handleEdit(row) {
      this.$router.push('/system/user/edit/' + row.id)
    },
    handleAssignRole(row) {
      // 打开分配角色对话框
      this.$refs.roleDialog.open(row)
    },
    handleDelete(row) {
      this.$confirm('确定要删除该用户吗？', '提示', {
        type: 'warning'
      }).then(async () => {
        await deleteUser(row.id)
        this.$message.success('删除成功')
        this.getList()
      })
    }
  }
}
</script>
```

#### 2. 分配角色对话框
```vue
<template>
  <el-dialog title="分配角色" :visible.sync="visible">
    <el-checkbox-group v-model="selectedRoles">
      <el-checkbox
        v-for="role in roleList"
        :key="role.id"
        :label="role.id"
      >
        {{ role.name }}
      </el-checkbox>
    </el-checkbox-group>
    <div slot="footer">
      <el-button @click="visible = false">取消</el-button>
      <el-button type="primary" @click="handleSubmit">确定</el-button>
    </div>
  </el-dialog>
</template>

<script>
import { getAllRoles } from '@/api/role'
import { getUser, updateUser } from '@/api/user'

export default {
  data() {
    return {
      visible: false,
      userId: null,
      roleList: [],
      selectedRoles: []
    }
  },
  methods: {
    async open(user) {
      this.visible = true
      this.userId = user.id
      
      // 获取所有角色
      const rolesRes = await getAllRoles()
      this.roleList = rolesRes.data
      
      // 获取用户已有角色
      const userRes = await getUser(user.id)
      this.selectedRoles = userRes.data.role_ids
    },
    async handleSubmit() {
      await updateUser(this.userId, {
        role_ids: this.selectedRoles
      })
      this.$message.success('分配成功')
      this.visible = false
      this.$emit('success')
    }
  }
}
</script>
```

---

## 📝 实际操作步骤

### 场景1: 创建新用户并分配权限

**步骤**:

1. **登录管理后台**
   - 访问: http://localhost:9527
   - 账号: admin / admin123

2. **进入用户管理**
   - 点击左侧菜单：系统管理 → 用户管理

3. **创建新用户**
   - 点击"新增用户"按钮
   - 填写信息：
     ```
     用户名：shop001
     密码：123456
     真实姓名：张三
     手机号：13800138001
     状态：启用
     ```

4. **分配角色**
   - 勾选角色：单店店长
   - 点击"确定"

5. **验证权限**
   - 登出当前账号
   - 使用新账号登录（shop001 / 123456）
   - 查看左侧菜单（只能看到单店店长的菜单）

---

### 场景2: 创建自定义角色

**步骤**:

1. **进入角色管理**
   - 系统管理 → 角色管理

2. **创建新角色**
   - 点击"新增角色"
   - 填写信息：
     ```
     角色名称：价格专员
     角色代码：price_staff
     描述：只能查看和管理价格
     ```

3. **分配菜单权限**
   - 展开菜单树
   - 勾选以下菜单：
     - ✅ 首页
     - ✅ 业务管理
       - ✅ 价格管理
   - 点击"确定"

4. **将角色分配给用户**
   - 进入用户管理
   - 找到目标用户
   - 点击"分配角色"
   - 勾选"价格专员"
   - 点击"确定"

---

### 场景3: 修改用户权限

**步骤**:

1. **查找用户**
   - 用户管理 → 搜索用户名

2. **编辑用户**
   - 点击"编辑"按钮

3. **修改角色**
   - 取消勾选：单店店长
   - 勾选：总部店长
   - 点击"保存"

4. **权限立即生效**
   - 用户下次登录时权限更新
   - 或通知用户重新登录

---

## 🎯 最佳实践

### 1. 角色设计原则

✅ **推荐做法**:
- 按职位创建角色（如：店长、店员、财务）
- 一个用户可以有多个角色
- 角色名称要清晰易懂
- 使用 code 标识符（如：shop_manager）

❌ **不推荐做法**:
- 为每个用户创建单独的角色
- 角色名称过于笼统（如：用户1、角色A）
- 频繁修改核心角色权限

### 2. 权限分配原则

✅ **最小权限原则**:
- 只分配必要的权限
- 新用户默认最小权限
- 定期审查权限

✅ **职责分离原则**:
- 财务只能查看，不能修改
- 店员不能看到敏感数据
- 超级管理员权限要慎用

### 3. 安全建议

🔒 **密码安全**:
- 强制6位以上密码
- 定期更换密码
- 使用 bcrypt 加密

🔒 **账号安全**:
- 限制登录失败次数
- 记录登录日志
- 异常登录告警

🔒 **权限安全**:
- 不要给普通用户超级管理员权限
- 定期清理无用账号
- 离职员工及时禁用账号

---

## 🔍 常见问题

### Q1: 用户看不到任何菜单？
**A**: 检查以下几点：
1. 用户是否分配了角色
2. 角色是否分配了菜单
3. 菜单状态是否启用（status=1）
4. 菜单是否可见（visible=1）

### Q2: 如何批量分配角色？
**A**: 可以通过循环调用 API：
```javascript
const userIds = [2, 3, 4, 5]
const roleId = 3

for (const userId of userIds) {
  await updateUser(userId, {
    role_ids: [roleId]
  })
}
```

### Q3: 删除角色后用户会怎样？
**A**: 
- 角色菜单关联会被删除
- 用户角色关联会被删除
- 用户下次登录时菜单会更新

### Q4: 可以跨店铺分配权限吗？
**A**: 
- 可以通过菜单实现
- 创建不同店铺的菜单目录
- 将对应店铺菜单分配给角色

---

## 📚 相关文档

- [API 接口文档](./API.md)
- [菜单管理指南](./菜单管理指南.md)
- [快速启动指南](./快速启动.md)

---

**文档版本**: v1.0.0  
**最后更新**: 2025-11-04  
**维护者**: AI Assistant





